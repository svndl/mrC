function [realT,realP,corrT,critVal,clustDistrib]= ttest_permute_sstats( inData,maxPerms,timeMask,permMatrix,deletePool,tParams )
        ...
        %   clustDistrib: distribution of significance run lengths for
         %                 permuted data.
% Latest modification Elham Barzegaran, 8/17/2018 -> to apply the cluster-based permutation test based on summary stats of the cluster not its length:
...
%%        
        ...
        [realT(timeMask),realP(timeMask),CI,realstats] = ttest(realData,0,tParams{:});
        ...
        parfor p = 1:maxPerms % all possible permutations
            ...
            clustDistrib(p) = max(clustStats(fakeT,stats.tstat)); % null distribution of largest summary stats clusters
        end
        critVal = prctile(clustDistrib,95); % 95 % of values are below this  
        realLocs = bwlabel(realT);   %identify contiguous ones
        realT2 = cat(find(max(size(realT))),0,realT,0);
        Clusters = [find(diff(realT2)==1) find(diff(realT2)==-1)-1];
        realCluster_sstat = abs(arrayfun(@(x) sum(realstats.tstat(Clusters(x,1):Clusters(x,2))),1:size(Clusters,1)));% summary statistics

        corrIdx = find(realCluster_sstat > critVal); % find clusters in real data bigger than 95% of null distribution
