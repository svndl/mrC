...
%         end
        ...
        function obj = mergROIs(obj,obj2)
            % Merge two class of ROIs together
        ...
        function obj = mergROIs2One(obj)
            % Merge ROIs within one class into one ROI
            if obj.ROINum==0
                return;
            end
            % merge atlas names, ROI names and their verticers
            TypeList = {obj.ROIList(:).Type};
            TypeList = unique(TypeList);
            roilist.Type = strcat(TypeList{:});
            
            HemiList = {obj.ROIList(:).Hemi};
            NameList = {obj.ROIList(:).Name};
            NameList = unique(cellfun(@(x,y) [x  y '_'],NameList,HemiList,'uni',false));
            roilist.Name = strcat(NameList{:});
            roilist.Hemi=[];
            
            meshInds =[obj.ROIList(:).meshIndices];
            [meshInds,mind] = unique(meshInds);
            roilist.meshIndices=meshInds;
            
            eccData = [obj.ROIList(:).eccData];
            if numel(eccData)==numel(meshInds)
                roilist.eccData = eccData(mind);
            else
                roilist.eccData = [];
            end
            
            roilist.Date = datestr(now);
            roilist.Comment = 'Generated by merging ROIs using ROIs.mergROIs2One';
            obj.ROIList = roilist;
        ...
        function [NameList] = getFullNames(obj,mode)
            ...
            elseif strcmp(mode,'noatlashemi')
                NameList = cellfun(@(y) [y],{obj.ROIList.Name},'UniformOutput',false);
            else
